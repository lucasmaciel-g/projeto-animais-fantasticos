export default function marcarFuncionamento() {
  ...
  const diaSemanaFuncionamento = funcionamento.dataset.semana
    .split(",")
    .map(Number);
  const horaFuncionamento = funcionamento.dataset.horario
    .split(",")
    .map(Number);
  ...
  if (
    diaSemanaFuncionamento.indexOf(diaSemanaAgora) !== -1 &&
